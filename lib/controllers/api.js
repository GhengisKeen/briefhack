...
exports.Reuters = function(req, res1) {
	...
	var a = url.format({
		pathname: '/msf1.0/data/' + req.params.api,
		search: req.params.paramString.replace(/\\(.?)/g, function(s, n1) {
			switch (n1) {
				case '\\':
					return '\\';
				case '0':
	...
	console.log("gonna get stuff", options);
	...
	https.get(options, function(res) {
		...
		res.on('data', function(chunk) {
			...
			var body = Buffer.concat(bodyChunks);
			console.log("Arrived here 3!");
			res1.send({
				// status: res.statusCode,
				// headers: res.headers,
				body: body.toString('utf-8')
			});
			console.log("did not arrived here!");
		}).error(function(err) {
			console.log(err);
		}).on('error', function(err) {

			console.log("Arrived here 4!");
			console.log(err);
		});
	});
};